    ...
    'custom-node',
    {
        draw (cfg, group) {
            ...
            if (imageMap[iconType]) {
                typeImageSource = imageMap[iconType][status] || imageMap[iconType].normal
            } else {
                // 默认类型图片，根据状态选择
                typeImageSource = status === 'abnormal'
...
G6.registerEdge('flowing-polyline', {
    ...
    calcPolylinePath (startPoint, endPoint, cfg) {
        ...
        if (cfg && cfg.sourceNode && cfg.targetNode) {
            ...
            // 判断源节点是否为负载均衡器
            const isSourceLoadBalancer = sourceModel.id === 'loadBalancer'
                                       || (sourceModel.label && sourceModel.label.includes('loadBalancer'))
            
            // 判断目标节点是否为主中心或灾备中心
            const isTargetCenter = (targetModel.id === 'mainCenter')
