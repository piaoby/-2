...
G6.registerNode(
    'custom-node',
    {
        draw (cfg, group) {
            ...
            const status = cfg.status // 正常/abnormal
            // const nodeId = cfg.id // 数据来源
            const iconType = cfg.iconType // 节点图标类型
            ...
            const imageMap = {
                host: {
                    normal: Base64Img.hostBase64,
                    abnormal: Base64Img.hostErrorBase64
                },
                F5: {
                    normal: Base64Img.F5Base64,
                    abnormal: Base64Img.F5ErrorBase64
                },
                oracle: {
                    normal: Base64Img.oracleBase64,
                    abnormal: Base64Img.oracleErrorBase64
                },
                backup: {
                    normal: Base64Img.backupBase64,
                    abnormal: Base64Img.backupErrorBase64
                }
            }

            // 根据图标类型和状态选择对应的图片资源
            if (imageMap[iconType]) {
                typeImageSource
          = imageMap[iconType][status] || imageMap[iconType].normal
            } else {
                // 默认类型图片，根据状态选择
                typeImageSource
          = status === 'abnormal'
        ...
        = alerts !== null && alerts !== '0' && alerts !== undefined
            ...
            const statusImageSize = size / 8
            const alertsTextWidth = status === 'abnormal' && showAlerts ? 25 : 0 // 只有在异常状态且有告警时才计算文字宽度
            const padding = 5 // 内边距
            ...
            const statusImage = group.addShape('image', {
                attrs: {
                    ...
                    img:
            status === 'abnormal'
                ? Base64Img.abnormalStatusBase64
                : // : status === 'construction'
                //     ? Base64Img.constructionStatusBase64
                Base64Img.normalStatusBase64
                },
                name: 'status-image'
            })

            // 如果是异常状态且有告警数，显示告警数（居中对齐）
            if (status === 'abnormal' && showAlerts) {
                // 告警数文本（在背景内居中）
                group.addShape('text', {
                    attrs: {
                        x: size / 8 + padding + statusImageSize + 10, // 图片右侧 + 间距
        ...
        update (cfg, item) {
            ...
            const labelHeight = 40 // 标签高度
            const alerts = cfg.Alerts || null // 获取告警数
            const showAlerts = alerts !== null && alerts !== '0' && alerts !== undefined
            ...
            if (children[0]) {
                ...
                if (status === 'abnormal') {
                    baseImageSource = Base64Img.abbaseImagelBase64
                }
            ...
            = status === 'abnormal'
                ...
                } else if (nodeId === 'loadBalancer' || nodeId === 'ESB集群信息') {
                    typeImageSource
            = status === 'abnormal'
                            ? Base64Img.F5ErrorBase64
                            : Base64Img.F5Base64
                } else {
                    typeImageSource
            = status === 'abnormal'
            ...
            // 更新状态背景
            const statusImageSize = nodeSize / 8
            const alertsTextWidth = status === 'abnormal' && showAlerts ? 25 : 0
            const padding = 5
            const totalContentWidth = showAlerts
                ? statusImageSize + alertsTextWidth
                : statusImageSize
            const backgroundWidth = totalContentWidth + padding * 2

            if (children[2] && children[2].get('name') === 'status-background') {
                children[2].attr({
                    x: nodeSize / 8 - 5,
                    y: -nodeSize / 2.4 - 5,
                    width: backgroundWidth,
            ...
            // 更新状态图片
            if (children[3] && children[3].get('name') === 'status-image') {
                children[3].attr({
                    x: nodeSize / 8 - 5 + padding,
                    y: -nodeSize / 2.4,
                    width: statusImageSize,
                    height: statusImageSize,
                    img:
            ...
            // 更新告警数文本
            if (status === 'abnormal' && showAlerts) {
                let alertTextFound = false
                for (let i = 0; i < children.length; i++) {
                    if (children[i].get('name') === 'alerts-text') {
                        children[i].attr({
                            x: nodeSize / 8 + padding + statusImageSize + 10,
            ...
            // 更新标签
            let labelIndex = 4
            // 需要考虑到可能不存在alerts-text的情况
            if (!showAlerts || status !== 'abnormal') {
                labelIndex = 4
            } else {
                labelIndex = 5
            }
            ...
            for (let i = 0; i < children.length; i++) {
                if (children[i].get('name') === 'custom-node-label') {
                    labelElement = children[i]
                    break
                }
            }
            
            if (cfg.label && labelElement) {
                labelElement.attr({
                    text: cfg.label,
                    fill: '#fff',
                    y: nodeSize / 2 + 15
            ...
            // 更新拖拽区域大小，包含标签区域
            const dragAreaIndex = children.length > 3 ? children.length - 1 : children.length - 1
            // 查找drag-area元素
            let dragAreaElement = null
            for (let i = 0; i < children.length; i++) {
                if (children[i].get('name') === 'drag-area') {
                    dragAreaElement = children[i]
                    break
                }
            }
...
G6.registerEdge('flowing-polyline', {
    ...
    // 检查是否为负载均衡器到主中心或灾备中心的连接
        if (cfg && cfg.sourceNode && cfg.targetNode) {
            ...
            // 判断源节点是否为负载均衡器
            const isSourceLoadBalancer
        = sourceModel.id === 'loadBalancer'
        || (sourceModel.label && sourceModel.label.includes('loadBalancer'))

            // 判断目标节点是否为主中心或灾备中心
            const isTargetCenter
        = targetModel.id === 'mainCenter'
        || targetModel.id === 'disasterCenter'
        || targetModel.id === 'dataCenter'
        || (targetModel.label
          && (targetModel.label.includes('主中心')
            ...
            // 检查是否为combo间的连接（通过判断节点是否有comboId或者parentId）
            const sourceHasCombo = sourceModel.type === 'custom-combo' || sourceModel.type === 'custom-parent-combo'
            const targetHasCombo = targetModel.type === 'custom-combo' || targetModel.type === 'custom-parent-combo'
            
            // 如果是combo之间的连接，使用带偏移量的折线路径
            if (sourceHasCombo && targetHasCombo) {
                // 获取偏移量，如果有的话
                let offset = 0
                if (cfg.style && cfg.style.offset) {
                    offset = cfg.style.offset
                }
                
                const midY = (startPoint.y + endPoint.y) / 2 + offset
                return [
                    ['M', startPoint.x, startPoint.y],
                    ['L', startPoint.x, midY],
                    ['L', endPoint.x, midY],
...
G6.registerCombo(
    'custom-combo',
    {
        draw (cfg, group) {
            ...
            const isDataCenterChild = cfg.parentId === 'dataCenter'
          ...
          = rows * nodeSize + (rows - 1) * nodeSpacing + 2 * padding + 50 // 额外50px用于combo标题
                ...
                // 其他combo保持原来的4列布局
                const nodesPerRow = isDataCenterChild ? 2 : 4
